...
if (!isset($_SESSION['usuario']) || !in_array($_SESSION['usuario'], $permitidos)) {
    echo "<script>
        ...
        window.location.href = 'http://localhost/TCC/loginadm.php';
    </script>";
    exit();
}
...
if (isset($_POST['cadastrar_cliente'])) {
	...
	if ($nome && preg_match('/^\d{11}$/', $cpf) && preg_match('/^\d{8,15}$/', $telefone) && $login && $senha) {
		$connection = new connection();
		$connection->insert('clientes', ['nome', 'cpf', 'telefone', 'login', 'senha'], [$nome, $cpf, $telefone, $login, $senha]);
	} else {
		echo "<script>alert('Verifique os campos: CPF e telefone devem conter apenas números.');</script>";
	}
}
...
if (isset($_POST['cadastrar_atendente'])) {
	...
	if ($nome && $login && $senha && in_array($cargo, ['garcom', 'adm'])) {
		$connection = new connection();
		$connection->insert('atendentes', ['nome_atendente', 'login', 'senha', 'cargo'], [$nome, $login, $senha, $cargo]);
	} else {
		echo "<script>alert('Preencha todos os campos e selecione um cargo válido.');</script>";
	}
}
?>
