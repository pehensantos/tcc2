...
if (!isset($_SESSION['usuario']) || !in_array($_SESSION['usuario'], $permitidos)) {
    echo "<script>
        ...
        window.location.href = 'http://localhost/TCC/logincliente.php';
    </script>";
    exit();
}
...
if (isset($_GET['destroy']) && $_GET['destroy'] == 'true') {
    ...
    header("Location: logincliente.php"); // Redireciona após logout
    exit;
}
			...
			if (isset($_POST['quantidade']) && isset($_POST['id_produto'])) {
			    ...
			    foreach ($quantidades as $i => $qtd) {
			        if (intval($qtd) > 0) {
			            ...
			            $comanda->confirmarPedido($_SESSION['id_comanda_atual'], (int)$ids[$i], $id_atendente, $id_cliente, "nao", (int)$qtd);
			        }
			    }
			}
		?>
